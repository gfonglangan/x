...
export function Button({ small= false, gray=false, className="", ...props}: ButtonProps){
    ...
    return <button className={`rounded-full text-white transition-colors duration-200 disable:cursor-not-allowed
    disabled:opacity-50 ${sizeClasses} ${colorClasses} ${className}`} {...props}></button>
}
